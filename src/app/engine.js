...
function buildCourse(){
  var course = [];
  var lessonNo = 0;              /* сквозной номер: по нему разводятся прикладные задания */
  UNITS.forEach(function(u){
    ...
    if(u.boss){
      ...
      var n = Math.max(1, Math.ceil(cards.length / 12));
      var per = PHRASES.length / n;
      for(var s = 0; s < n; s++){
        lessons.push({id:u.id+".L"+s, t:"Урок "+(s+1),
                      kind:"slang",
                      from:Math.round(s*per), to:Math.round((s+1)*per),
                      cards:cards.slice(s*12, s*12+12)});
      }
      lessons.push({id:u.id+".LX", t:"Проверка", kind:"check", cards:cards, quiz:[], build:false});
    } else {
      /* По 8 карточек на урок, а не по 5: путь был вдвое длиннее нужного,
         а карточка успевала повториться до семи раз за один проход. */
      for(var i2 = 0; i2 < cards.length; i2 += 7){
        var chunk = cards.slice(i2, i2+7);
        /* хвост короче четырёх карточек присоединяем к предыдущему уроку */
        if(chunk.length < 4 && lessons.length){
          lessons[lessons.length-1].cards = lessons[lessons.length-1].cards.concat(chunk);
          break;
        }
        lessons.push({id:u.id+".L"+(i2/7), t:"Урок "+(lessons.length+1), kind:"cards", cards:chunk,
                      quiz: quizzes.length ? [quizzes[(i2/7) % quizzes.length]] : []});
      }
      lessons.push({id:u.id+".LX", t:"Проверка", kind:"check", cards:cards, quiz:quizzes, build:!!u.build});
    }
    lessons.forEach(function(l){ l.n = lessonNo++; });
...
function variantFor(card, srs, rnd, role){
  var s = seenTitles(srs)[card.t];
  /* Вопрос «что такое X» решается подбором слова: определение
     почти всегда повторяет сам термин. Он годится для знакомства, но не
     для проверки. Раньше при пустом интервальном повторении — то есть весь
     первый проход по пути — он выпадал каждый раз, и одна и та же карточка
     до семи раз задавала дословно один вопрос. */
  if(role === "review") return [3, 1, 2][Math.floor(rnd() * 3)];
  if(!s) return (card.fact && rnd() < 0.45) ? 3 : 0;
  var box = s.b || 0;
  var pool = box >= 3 ? [2, 3, 1] : [3, 1, 2];
...
/* Прикладные задания берутся из маленьких пулов: 20 задач, 12 сборок,
   8 пропусков. Случайный выбор с защитой только внутри урока давал одну
   и ту же сборку до десяти раз за проход. Теперь это ротация по сквозному
   номеру урока: повтор начинается не раньше, чем пул закончится. */
function appliedFor(unit, lesson, rnd, used, slot){
  var out = [];
  var pos = (lesson.n || 0) + (slot || 0);
  var qs = QUIZ.map(function(q, i){ return {q:q, i:i}; })
               .filter(function(x){ return unit.quizG.indexOf(x.q.g) >= 0 && !used["q|" + x.i]; });
  if(qs.length){ var q = qs[pos % qs.length]; out.push(genQuiz(q.q, q.i, rnd)); used["q|" + q.i] = 1; }
  ...
  /* Свои по теме идут первыми, но если их меньше трёх, пул добирается
     остальными. Иначе ротация по пулу из одного элемента возвращает его же
     в каждом уроке раздела. */
  var topics = unit.quizG.concat(unit.groups);
  var ownB = BUILD.filter(function(b){ return topics.indexOf(b.g) >= 0; });
  var bs = ownB.length >= 3 ? ownB
         : ownB.concat(BUILD.filter(function(b){ return topics.indexOf(b.g) < 0; }));
  var allZ = CLOZE.map(function(c, i){ return {c:c, i:i}; }).filter(function(x){ return !used["z|" + x.i]; });
  var ownZ = allZ.filter(function(x){ return topics.indexOf(x.c.g) >= 0; });
  var cz = ownZ.length >= 3 ? ownZ
         : ownZ.concat(allZ.filter(function(x){ return topics.indexOf(x.c.g) < 0; }));
  if(bs.length && (pos % 2 === 0 || !cz.length)){
    var b = bs[Math.floor(pos / 2) % bs.length];
    out.push(genBuild(b)); used["b|" + b.task] = 1;
  } else if(cz.length){
    var z = cz[Math.floor(pos / 2) % cz.length];
    out.push(genCloze(CLOZE[z.i], z.i, rnd)); used["z|" + z.i] = 1;
  }
  return out;
}
...
function makeLesson(unit, lesson, attempt, ctx){
  ...
  core.forEach(function(c){ ex.push(tag(genFromCard(c, CARDS, variantFor(c, srs, rnd, "core"), rnd), c.g)); });
  ...
  /* Прикладное добавляется ДО подсчёта: оно помечено той же темой,
     что и ядро, и раньше раздувало её уже после того, как посчитано,
     сколько нужно разбавления. Оттуда брались тройки одной темы подряд. */
  appliedFor(unit, lesson, rnd, used).forEach(function(x){ ex.push(tag(x, unit.quizG[0] || coreGroups[0])); });
  if(lesson.kind === "check"){
    appliedFor(unit, lesson, rnd, used, 1).forEach(function(x){ ex.push(tag(x, unit.quizG[0] || coreGroups[0])); });
  }
  ...
  /* Сколько нужно повторения, чтобы разбавить самую массивную тему урока:
     ...
     Плюс одно сверху на пары и вопрос с собеседования, которые добавятся ниже. */
  var counts = {}, domin = 0;
  ...
  var reviewN = Math.max(lesson.kind === "check" ? 4 : 3,
                         Math.min(14, domin - 1 - otherN));
  var review = pickReview(core, reviewN, rnd, srs, coreGroups);
  review.forEach(function(c){ ex.push(tag(genFromCard(c, CARDS, variantFor(c, srs, rnd, "review"), rnd), c.g)); });
